...
[Q,\f(x)] &= q \f(x).
\ee
The three-point function $\<\f^* \f J\>$ is fixed by conformal symmetry up to a coefficient:
\be
...
To simplify the computation, it is useful to study the configuration
\be
\label{eq:ourconfig}
x_1=\oo,\quad x_2=0,\quad x_3=x.
\ee
Recall that $\f^*(\oo) \equiv \lim_{L\to \oo} L^{2\De_\f} \f^*(Le)$, where $e$ is a unit vector. In this configuration, we have
\be
\label{eq:chargeintegral}
q &= i\int_\Sigma dS_\mu(x) \frac{f_{\f^*\f J} x^\mu }{x^{d}} \nn\\
 &= i S_d f_{\f^*\f J},
\ee
where $S_d=\frac{2\pi^{d/2}}{\Gamma(d/2)}$ is the volume of the unit sphere in $d$-dimensions. The fact that the integral (\ref{eq:chargeintegral}) only depends on the topological class of $\Sigma$ is simply Gauss' law from electrostatics.

We can summarize this result by writing the $\f$ term in the $J\x\f$ OPE:
\be
J^\mu(x) \f(0) &= -\frac{i}{S_d}q \frac{x^\mu}{x^d} \f(0) + \dots.
\ee
Here ``$\dots$" represents descendants of $\f$ and other conformal multiplets. Taking the correlator of both sides with $\f^*(\oo)$, we get back to (\ref{eq:threeptcurrent}) in the configuration (\ref{eq:ourconfig}). Integrating $J^\mu$ around a sphere $\Sigma$ centered at $\f$ gives $Q(\Sigma)\f(0)=q\f(0)$.
...
[Q,\f_i(x)] &= t_i{}^j \f_j(x),
\ee
for some matrix $t_i{}^j$. Note that $T_i{}^j$ can only be nonzero if $\De_i=\De_j$, by dimensional analysis.
Repeating the above computation, we find
\be
\label{eq:jope}
J^\mu(x) \f_i(0) &= -\frac{i}{S_d} t_i{}^j \frac{x^\mu}{x^d} \f_j(0) + \dots.
\ee
The ``$\dots$" cannot contain other scalar primary operators. Otherwise, by integrating $J$ over a sphere, we would find a contradiction with (\ref{eq:generalizedQ}). Furthermore, the ``$\dots$" cannot contribute to the integral of $J^\mu$ over a sphere. Otherwise, $Q$ would transform $\f_i(0)$ into a non-scalar operator.

In general, if $\cO_i$ have spin, there can be multiple three-point structures for $\<\cO_i \cO_j J\>$. One of these three-point structures will be fixed by the Ward-identity, and the others will be unconstrained. For example, given a collection of currents $J_\mu^a$ generating a nonabelian symmetry with structure constants $f^{abc}$, the three-point function will have the form
\be
...
Some OPE coefficients of the stress-tensor are also fixed by Ward identities in a similar way. Again, there is only a single conformally-invariant structure for the three-point function $\<\f_i \f_j T^{\mu\nu}\>$. The integral of $T^{\mu\nu}$ gives the conformal charges acting on a two-point function, so $f_{ij T}$ must be proportional to $C_{ij}=\<\f_i \f_j\>$.
...
\ee
where $f_{Ti}{}^k C_{kj}=f_{Tij}$. One can derive this expression by taking the correlator of both sides with another scalar and using (\ref{}). However, a shortcut is to use dimensional analysis and tracelessness of $T^{\mu\nu}$ to fix the form that the OPE can take. One can check that the result is automatically conserved (away from the origin).

Note that the dilatation operator is given by
\be
D &= - \int_\Sigma dS_\mu x_\nu T^{\mu\nu}.
\ee
...
As before, this result shows that no other scalars can appear in the $T\x \f_i$ OPE. Operators with spin can appear, but the integrals of their corresponding terms over the sphere must vanish. When $\cO_i$ have spin, there can be multiple three-point structures for $\<\cO_i \cO_j T^{\mu\nu}\>$. Generically, one of them is fixed by Ward identities to be proportional to $\De_i$ and another is fixed to be related to the rotation generators $\cS_{\mu\nu}$. (This latter type of Ward identity is trivial in the scalar case.) The other structures are unconstrained.
...
\be
\<J^\mu(x)J_\nu(0)\> &= C_J \frac{I^{\mu}{}_{\nu}(x)}{x^{2(d-1)}} \nn\\
\<T^{\mu\nu}(x) T_{\rho\s}(0)\> &= C_T \frac{I^{(\mu}{}_{\rho}(x) I^{\nu)}{}_{\s}(x)-\frac{1}{d} \de^{\mu\nu}\de_{\rho\s}}{x^{2d}}.
\ee
The two-point coefficients $C_J, C_T$ are interesting physical observables. 
