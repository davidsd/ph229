...
These introductory notes are heavily based on Silviu Pufu's Bootstrap 2017 lectures \cite{}, John Cardy's book {\it Scaling and Renormalization in Statistical Physics\/}, Slava Rychkov's EPFL lectures on CFT \cite{}, and John McGreevy's lectures on QFT \cite{}.
...
\subsection{The $O(N)$-model and the large-$N$ expansion}
...
It is instructive to see how the dimension of $\f^2$ changes during the flow from the Gaussian point to the nontrivial critical point. One way is to study the two-point function
\be
F(x) &= \<\f^2(x) \f^2(0)\>,
\ee
...
The simplest contribution to $F(x)$ comes from a bubble diagram
\be
\<\f^2(x)\f^2(0)\> &\supset 2N G(x)^2.
\ee
The factor $N$ comes from summing over $i$ indices for the two propagators. These must be equal because of the contractions in $\f^2$, so we get a single sum over $N$. In Fourier-space, the bubble diagram becomes
\be
...
We will be interested in a limit where $N\to \oo$ with the product $uN$ held fixed. In this limit, chains of bubbles are the only diagrams that contribute. Any other diagram we might draw requires reconnecting different $O(N)$ indices in a way that lowers the resulting powers of $N$. Thus, in this limit, we simply have to sum bubble diagrams, which form a geometric series
\be
...
For higher-order computations, we can write a set of effective Feynman rules that summarize the result we found. We have a $\f_i\f_j$ propagator of the usual form $\frac{\de_{ij}}{p^2}$, and a dotted line that represents a geometric series of bubble diagrams that behaves like $|p|$.
...
\subsection{Banks-Zaks fixed-points}
