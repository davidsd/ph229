...
\textbf{\textit{Note: This section is somewhat technical, and can be skipped on a first reading.}}
...
\ee
The tensor $g_{a\bar a}(x)$ is the inverse of the numerator of a two-point function
\be
...
\ee
where $c_\cO$ is a positive operator-dependent constant.
...
\ee
where $y=x/x^2$.
We have given $\<\cO_a|$ a lowered index because it transforms in the dual representation $\rho^*$. One very nice property of BPZ conjugation is that the limit $\<\cO_a|$ is independent of the direction along which we take $\cO^\dag$ to infinity. We can see this by computing an inner product
\be
