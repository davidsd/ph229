...
For operators with spin, the solutions to the conformal Ward identities are more interesting. We will discuss two different approaches, both with advantages and disadvantages. The first approach is to relate operators at arbitrary positions to some standard configuration (``conformal frame"), where the Ward identities are easier to solve. This is the same type of argument we used above to show that a four-point function can depend on two conformal cross-ratios.
...
We can construct the stabilizer group as follows. Consider an inversion $I:x\mapsto \frac{x}{x^2}$. This fixes $e$, but moves the origin to $\oo$. Now, we can perform an arbitrary rescaling and rotation around $e$ without moving either $e$ or $\oo$:
\be
x^\mu &\mapsto e^\mu + \l^{-1} h^\mu{}_\nu (x - e)^\nu,\qquad \l\in\R^+,\ h\in \SO(d).
\ee
Finally, we can invert again to map infinity back to the origin. Overall, we have constructed the map
\be
\label{eq:mapdefinition}
x\mapsto x'(x) = I(e + \l^{-1} h(I(x) - e)),\qquad I(x) = \frac{x}{x^2},
\ee
parameterized by $\l,h$. Note that because we have used $I$ twice, this map is in the connected component of the identity of $\SO(d+1,1)$, and is thus always a symmetry in a CFT. We have found that the stabilizer group of two points is $\R^+ \x \SO(d) \subset \SO(d+1,1)$.
...
x'(x) &\sim \l h^\mu{}_\nu x^\nu + O(x^2),
\ee
which looks locally like a rescaling by $\l$ and a rotation by $h$.

Thus, the Ward identity (\ref{eq:specializedtwoptward}) implies
\be
...
For traceless symmetric tensor operators $\cO_1^{\mu_1\cdots\mu_\ell}$ and $\cO_2^{\mu_1\cdots\mu_{\ell'}}$, it's not hard to show that the two-point function can only be nonvanishing if the spins are equal $\ell=\ell'$. In that case, the two-point function takes the form
\be
...
\be
(\Box \otimes \Box \otimes \mathbf{1})^{\SO(d-1)} &= (\mathbf{1} \oplus \Box)\otimes (\mathbf{1} \oplus \Box) \otimes \mathbf{1} \nn\\
&= 2\mathbf{1} \oplus \dots.
\ee
We could compute the correlator in a general configuration using the Ward identities. Tracing everything through, we would find
\be
...
The central observation is that the conformal group is the same as $\SO(d+1,1)$ (exercise~\ref{}), which acts simply by matrix multiplication on $\R^{d+1,1}$
\be
...
\ee
The null cone is preserved by $\SO(d+1,1)$, so we have a $d+1$-dimensional space on which the conformal group acts. To finally get to $d$-dimensions, we can projectivize: we mod out by rescalings,
\be
\textrm{projective null cone} = \frac{\{X:X^2 = 0\}}{X\sim \l X\ \ (\l\in \R)}.
\ee

In the projective null cone, each null vector $X$ represents an equivalence class, and the conformal group maps one equivalence class to another. We can think of rescaling as a ``gauge redundancy." To be more explicit, let us fix a gauge by choosing a representative for each equivalence class. The most important gauge fixing for us will be $X^+=1$, which defines the ``Poincare section" of the null cone. Given a null vector $X=(X^+,X^1,X^\mu)$, we can rescale by $1/X^+$ to obtain an equivalent vector $(1,X^-/X^+,X^\mu/X^+)$ on the Poincare section. (The only case in which this doesn't work is when $X^+=0$, so that $X$ is equivalent to $(0,1,0)$. We will return to this case in a moment.)

Every null vector on the Poincare section can be written
\be
X &= (1,x^2, x^\mu),
\ee
where $x\in \R^d$ and $x^2=x_\mu x^\mu$. Now consider the action of the conformal group on $X$. We have $X \mapsto g X$. However, $gX$ is not on the Poincare section in general, so we must rescale by $(gX)^+$ to restore our gauge choice
\be
X &\mapsto g X \to (gX)/(gX)^+ = (1,x'^2,x'^\mu)
\ee
Following this chain gives a map $x\to x'$ from $\R^d$ to itself. We claim that this is exactly the usual action of the conformal group.

To see this, let us consider some examples. One type of element of $\SO(d+1,1)$ are simply $\SO(d)$ transformations acting on the coordinates $X^\mu$,
\be
g=\begin{pmatrix}
1 & 0 & 0 \\
0 & 1 & 0 \\
