...
It may not be immediately obvious that $g_a$ preserves the metric (\ref{eq:embeddingmetric}), but it's easy to check that $g_a X$ is null, which implies that it does. Note that $g_a$ also does not take us off the Poincare section, so it just gives the transformation $x^\mu \mapsto x^\mu + a^\mu$, which is a translation.
...
\Omega(x')^{-1}=(g X)^+
\ee
Since this is also trivially true for translations and rotations, it is true for all $g\in \SO(d+1,1)$.
...
\Phi(\l X) &= \l^{-\De} \Phi(X).
\ee
Together, these conditions (defined on the null cone, homogeneous) ensure that $\Phi(X)$ only depends nontrivially on $d$ coordinates (instead of $d+2$ coordinates), as we should expect from the fact that it is encoding a $d$-dimensional operator.
