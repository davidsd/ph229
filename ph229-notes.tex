...
\subsection{Scale implies conformal invariance in 2d}

We have seen that critical points exhibit scale invariance at long distances, but it is not obvious why this implies tracelessness of the stress tensor. In general spacetime dimensions, proving that scale invariance implies $T^\mu_\mu=0$ is an open problem.\footnote{An almost-complete argument exists in 4-dimensions \cite{}. Part of the challenge is identifying the appropriate assumptions to make. There exist boring/pathological counterexamples in 3d (for example free electromagnetism), and the right formulation of the theorem should exclude these cases.} However, the proof is straightforward in 2d, so let us give it.
...
In a {\it unitary} quantum field theory,\footnote{Roughly-speaking, unitary means a theory that can be quantized in such a way that the space of states has a positive-definite inner product and the Hamiltonian is Hermitian and bounded from below. We will discuss unitarity in more detail later.} if the two point function of an operator vanishes identically, the operator itself must vanish. The argument is as follows. Remember that a two-point function can be written as a sum over states with positive coefficients (\ref{eq:completeset})
\be
\<T(x^0,\mathbf{ 0})T(0)\> &= \sum_\psi |\<0|T(0)|\psi\>|^2 e^{-E_\psi x^0}.
\ee
If the two-point function vanishes, then each coefficient must vanish: $\<0|T(0)|\psi\>=0$ for all $\psi$. Now consider a general correlation function including $T(x)$,
\be
\<T(x) \cO_1(x_1)\cdots \cO_n(x_n)\>.
\ee
Using $\<0|T(0)|\psi\>=0$, this correlator vanishes if $x$ is in the past or future of all the other $x_i$. However, correlation functions are analytic in their arguments, and the analytic continuation of zero is zero. Thus all correlators of $T(x)$ vanish, and $T(x)$ vanishes as an operator.

\subsection{Symmetries from tracelessness}

Recall that the charge $Q_\e(\Sigma)$ will be conserved if
\be
 \frac 1 2(\ptl_\mu \e_\nu+\ptl_\nu \e_\mu) T^{\mu\nu} &= 0.
 \ee
 When the stress tensor is traceless, this condition is solved by a less constraining version of the Killing equation (\ref{eq:killingvector}),
\be
...
Equation (\ref{eq:conformalkilling}) is the {\it conformal\/} Killing equation. Let us find its solutions on $\R^d$. Taking $\ptl^\nu\ptl^\mu$ of both sides, we find
\be
...
\ee
For the moment, let us focus on the case $d\neq 2$. Taking two derivatives of the conformal Killing equation and using (\ref{eq:equationaboutdivergence}), we find
\be
...
\ee
These are three equations for three unknowns, so it follows that
\be
\ptl_\a\ptl_\b \ptl_\g \e_\de &= 0.
\ee
Thus $\e_\mu$ is at most a quadratic polynomial in $x$.
...
\end{align}
The corresponding symmetry charges are $D=Q_d$ and $K_\mu=Q_{k_\mu}$.

Let us now return to the case $d=2$. It is natural to write the conformal Killing equation in terms of holomorphic and antiholomorphic coordinates $\e = \e \ptl_z + \bar \e \ptl_{\bar z}$, where it becomes
\be
\ptl_z \bar \e = 0,\quad \ptl_{\bar z} \e = 0.
\ee
(The equation involving $\ptl_z \e + \ptl_{\bar z} \bar \e$ is tautological.) Thus, we can solve the conformal Killing equation by choosing $\e$ ($\bar\e$) to be any holomorphic (antiholomorphic) function,
\be
\e &= \e(z) \ptl_z + \bar \e(\bar z) \ptl_{\bar z}.
\ee
Consequently, 2d CFTs have an infinite set of conserved charges given by the integral of the stress tensor against any holomorphic or antiholomorphic vector field. These infinite symmetries will play an important role later when we discuss the conformal bootstrap in 2d.
