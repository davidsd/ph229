...
\de_\w \<\cdots\>_g &= -\p{\int d^2 x \sqrt g\, \w(a_1 R + a_2 R^2 + a_3\nabla^2 R + \dots)}\<\cdots\>_g.
\ee
...
\de_\w\mathcal{W}[g] &= \int d^2 x \sqrt g\, \w(a_1 R + a_2 R^2 + a_3 \nabla^2 R + \dots),\\
...
\ee
Applying this condition to $\mathcal{W}[g]$, we get constraints on the coefficients $a_i$. For example, $a_1$ is unconstrained because
\be
...
\be
\mathcal{W}[g] &\sim \mathcal{W}[g] + \int d^2 x \sqrt g (b_1 R + b_2 R^2 + \dots).
\ee
Taking the variation of both sides, we find that we can shift $a_2$, but not $a_1$.

In general, addition of arbitrary counterterms allows us to remove any terms in (\ref{eq:weylvariationpartition}) that are not consistent with dimensional analysis. We could have simplified our discussion by only including the first term in (\ref{eq:weylvariationpartition}).

\end{itemize}

Thus, the space of consistent Weyl anomalies is the solution to a kind of cohomology problem. The only nontrivial coefficient is $a_1$, which we can fix by matching to (\ref{eq:Weylanomaly}). We find
\be
...
Integrating the Weyl variation (\ref{eq:weylvariationinfinitesimal}), we find
\be
\<\cdots\>_{e^{2\w} g} &= e^{S_{\mathrm{Weyl}}[g,\w]}\<\cdots\>_g.
\ee
For the discussion in this section, we have considered Weyl variations that are trivial near operator insertions. If we now allow them to become nontrivial, we get nontrivial Weyl factors depending on the dimensions of the operators. For example, for primary scalars $\cO_i$, we have
\be
...
\ee
In general, the Hamiltonian on the cylinder is equal to the flat-space dilatation operator, with an additional shift by the Casimir energy
\be
H_\mathrm{cyl.} &= D - \frac{c}{12}.
\ee
This shift doesn't affect any of our previous analysis of correlation functions on the cylinder because the Casimir cancels from the position dependence of operators
\be
...
Now that we understand the Hamiltonian on the cylinder $S^1 \x \R$, let us compute the partition function of a 2d CFT on the torus $T^2$.
