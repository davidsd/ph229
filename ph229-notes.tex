...
\begin{abstract}
Notes for Physics 229, 2017-2018. These notes are in progress. They were last updated on \today. In a few places, there are reminders \draftnote{in blue} to the author to add additional discussion/comments. There may also be empty citations and missing figures.
\end{abstract}
\body
...
These introductory notes are heavily based on Silviu Pufu's Bootstrap 2017 lectures \cite{}, John Cardy's book {\it Scaling and Renormalization in Statistical Physics\/} and John McGreevy's lectures on QFT \cite{}.
...
Our first examples of critical points occur in magnets. The discussion in this section borrows heavily from Cardy \cite{}. Given a magnet with temperature $T$, we can apply a magnetic field $\vec{H}$ and measure the magnetization $\vec{M}$. There are three main types of magnets in 3-dimensions, which are distinguished by their symmetries:\footnote{Here, we mean non-spacetime symmetries, usually called ``global" or ``flavor" symmetries. The emergent spacetime symmetry group is still the Poincare group, or the conformal group at the critical point.}
\begin{itemize}
...
\hfill\draftnote{\it End of lecture 5}

\section{CFTs in perturbation theory}

\subsection{Scaling and renormalization}
...
At a fixed-point, our coarse-graining rules fix the two-point function
\be
...
\ee
These combinations are invariant under the transformation (\ref{eq:coarsegrainingtransf}). Thus, invariance under coarse-graining means that correlators in terms of $\cO_A,\l_A$ are independent of the cutoff $a$. The tradeoff is that correlators must now also be consistent with dimensional analysis under which $\cO_A$ has length-dimension $-\De_A$ and $\l_A$ has length-dimension $-y_A = \De_A-d$. For example, at the fixed-point $\l_A=0$, a two-point function of $\cO_A$ must be given by
\be
\<\cO_A(x_1) \cO_A(x_2)\> &\propto \frac{1}{|x_1-x_2|^{2\De_A}}.
\ee
The right-hand side is the only quantity consistent with Poincare symmetry and dimensional analysis. (Of course, it also follows trivially from (\ref{eq:coarsegrainingtwopt}) and (\ref{eq:redefineops}).)
...
For concreteness, let us focus on the Ising model. We first set the irrelevant $\l_A$'s to zero so we can later treat them as small perturbations. Let us also tune parameters in the UV so that $\l_h=0$ (in the case of a magnet, we switch off the external field). There is now exactly one relevant parameter turned on, namely $\l_t$. We can associate a mass scale to it given by
\be
m_t &= |\l_t|^{1/y_t} = |u_t|^{1/y_t} a^{-1}.
\ee
By dimensional analysis, any other dimensionful quantity must be proportional to this mass scale. For example, the mass gap of the theory in the IR must be
\be
\label{eq:massgapeq}
m_\mathrm{gap} &\propto m_t \propto |u_t|^{1/y_t} \propto |t|^{1/y_t}.
\ee
This determines the critical exponent 
\be
\nu=\frac{1}{y_t} = \frac{1}{d-\De_t}.
\ee

Note that the constant of proportionality in (\ref{eq:massgapeq}) can depend on the sign of $t$. The reason is that, depending on the sign of $t$, the theory may flow to different theories in the IR (figure~\ref{}). For example, in the Ising model, when $t>0$ the theory flows to a gapped phase where $\Z_2$ is unbroken, whereas when $t<0$, the theory flows to a gapped phase that spontaneously breaks $\Z_2$. No matter where the theory flows, $m_t$ is still the only mass-scale around, so $m_\mathrm{gap}$ must be proportional to $m_t$. However, the actual dynamics that determines $m_\mathrm{gap}$ can be arbitrarily complicated and generally depends in a nontrivial way on the actual IR theory. 

\hfill \draftnote{\it End of lecture 6}



%Consider an RG flow between a microscopic theory QFT$_\mathrm{UV}$ with some length cutoff $a$ and some SFT$_\mathrm{IR}$ at long distances. For the present purposes, scale-invariance means that there exists a transformation that rescales space $x\to bx$ and local operators $\cO_i \to b^{\De_i} \cO_i$, such that correlation functions in the SFT$_\mathrm{IR}$ are invariant,
%\be
%\<\cO_1(x_1)\cdots \cO_n(x_n)\> &= b^{\sum_i \De_i} \<\cO_1(b x_1)\cdots \cO_n(b x_n)\>.
%\ee
%The existence of this symmetry means that we can do dimensional analysis where we treat $\cO_i$ as having length-dimension $-\De_i$. 
%
%Now consider perturbing the action of the UV theory by some operator $\cO_\mathrm{UV}$ with a small coupling-constant $g$,
%\be
%\label{eq:uvdeformation}
%S_\mathrm{UV} &\to S_\mathrm{UV} + g\int d^d x\, a^{\Delta_\mathrm{UV} - d} \cO_\mathrm{UV}(x).
%\ee
%Here we have included factors of the length cutoff to make $g$ dimensionless.\footnote{In the case of a lattice model like the Ising model, all UV operators are dimensionless $\De_\mathrm{UV}=0$, because they're just products of classical dimensionless variables (e.g.\ spins $s_i$). The factor of $a^{-d}$ in (\ref{eq:uvdeformation}) comes from writing the sum over lattice sites $i\in \Z^d$ as an integral over space $\sum_{i\in \Z^d} \to \int \frac{d^d x}{a^d}$.} As long as the coupling $g$ is small, the perturbed theory should still be describable within the IR effective theory, with an action
%\be
%S_{\mathrm{SFT}_\mathrm{IR}} + \int d^d x \sum_i c_i(g) a^{\De_i - d} \cO_i(x).
%\ee
%Because $\mathrm{SFT}_\mathrm{IR}$ has no intrinsic scale, the only thing we can insert to make the action dimensionless are powers of the UV length cutoff. The dimensionless coefficients $c_i(g)$ are determined by performing a matching computation in the sense of effective field theory --- i.e.\ we compute observables in both theories and systematically match expansions in $a/x$. This matching computation will give $c_i(g)$ that are analytic in $g$,
%\be
%c_i(g) &= c_{i0} + c_{i1} g + c_{i2} g^2 + \dots.
%\ee
%
%
%
%
%effective theory QFT$_\mathrm{IR}$ at long distances. For example, QFT$_\mathrm{UV}$ could be a discrete lattice model with lattice spacing $a$ and QFT$_\mathrm{IR}$ could be some continuum theory.
%
%In general, given such an RG flow, we have a linear map between local operators in the UV theory and local operators in the IR theory, given by simply reinterpreting the UV insertion in terms of IR degrees of freedom. Specifically, given $\cO_\mathrm{UV}(x)$, we can insert it in the path integral and consider correlation functions where all distances are large compared to the UV cutoff $x\gg a$. Such correlators are described by the IR theory, so we must have an operator equation
%\be
%\label{eq:uvirmap}
%\cO_\mathrm{UV} &= \sum_i d_i \cO_i.
%\ee
% Here, $i$ runs over local operators of the IR theory and $d_i$ are coefficients that depend on the details of the RG flow.
%Equation (\ref{eq:uvirmap}) is valid when all other operators are far from $\cO_\mathrm{UV}$.
%
%Now suppose that the IR theory is a scale-invariant QFT (SFT). For the present purposes, scale-invariance means that there exists a transformation that rescales space $x\to bx$ and local operators $\cO_i \to b^{\De_i} \cO_i$, such that correlation functions in the SFT$_\mathrm{IR}$ are invariant,
%\be
%\<\cO_1(x_1)\cdots \cO_n(x_n)\> &= b^{\sum_i \De_i} \<\cO_1(b x_1)\cdots \cO_n(b x_n)\>.
%\ee
%The existence of this symmetry means that we can do dimensional analysis where we treat $\cO_i$ as having length-dimension $-\De_i$.  By dimensional analysis, the map (\ref{eq:uvirmap}) must take the form
%\be
%\label{eq:sftreinterpmap}
%\cO_\mathrm{UV} &= \sum_i c_i a^{\De_i - \De_\mathrm{UV}} \cO_i,
%\ee
%where $\De_\mathrm{UV}$ is the mass-dimension of $\cO_\mathrm{UV}$ in the UV theory. The reason $a$ appears is that the IR theory has no intrinsic scale, so the only possibility is the characteristic scale of the UV theory. The $c_i$ are now dimensionless.
%
%Finally, consider adding $\cO_\mathrm{UV}$ as a small perturbation to the action of the UV theory
%\be
%\label{eq:uvdeformation}
%S_\mathrm{UV} &\to S_\mathrm{UV} + g\int \frac{d^d x}{a^d}\, a^{\De_\mathrm{UV}}\cO_{UV}(x).
%\ee
%Here, we have included factors of the length cutoff to make the action dimensionless.\footnote{In the case of a lattice model like the Ising model, all UV operators are dimensionless $\De_\mathrm{UV}=0$, because they're just products of classical dimensionless variables (e.g.\ spins $s_i$). The factor of $a^{-d}$ in (\ref{eq:uvdeformation}) comes from writing the sum over lattice sites $i\in \Z^d$ as an integral over space $\sum_{i\in \Z^d} \to \int \frac{d^d x}{a^d}$.}  In the IR theory, this corresponds to
%\be
%\label{eq:irdeformation}
%S_\mathrm{IR} &\to S_\mathrm{IR} + g\int d^d x \sum_i c_i a^{\De_i-d} \cO_i(x) \\
%&= S_\mathrm{IR} + \int d^d x \sum_i \frac{\cO_i(x)}{m_i^{\De_i - d}},
%\ee
%where
%\be
%m_i=(g c_i)^{\frac{1}{d-\De_i}} a^{-1}
%\ee
%is the mass scale associated with the operator $\cO_i$.\footnote{Here, we have glossed over an important subtlety. The operator equation (\ref{eq:sftreinterpmap}) is only valid when insertions of $\cO_\mathrm{UV}$ are far apart. However, when we add $\cO_\mathrm{UV}$ to the action, we are including configurations where UV operators are on top of each other (e.g.\ in perturbation theory). Coincidences between operators result in divergences in the IR theory which must be regularized by introducing a cutoff and adding counterterms. There will be precisely one choice of counterterms that matches the UV perturbation (\ref{eq:uvdeformation}). Consequently, the $c_i$ in (\ref{eq:irdeformation}) depend on the regularization scheme of the IR theory and are not necessarily the same as the $c_i$ in (\ref{eq:sftreinterpmap}). However, dimensional analysis is still correct.} Finally, let us split the sum over IR operators into relevant and irrelevant contributions (for simplicity, we will assume that no IR operators are marginal $\De_i=d$),
%\be
%\label{eq:splitiraction}
%\p{S_\mathrm{IR} + \int d^d x \sum_{\De_i < d} m_i^{d-\De_i} \cO(x)} + \sum_{\De_i > d} \frac{\cO_i(x)}{m_i^{\De_i - d}}.
%\ee
%
%
%We will be interested in the limit of very small perturbations $g\ll 1$. As $g\to 0$, the irrelevant mass scales increase, and consequently the irrelevant contributions in (\ref{eq:splitiraction}) can be treated as small perturbations to the action in parentheses.
%
%The action in parentheses describes an SFT deformed by a finite number of relevant operators.\footnote{In a CFT, we will prove that the number of relevant operators is finite in section~\ref{}.} For simplicity, consider the case where 
%
%
%The significance of the mass scales $m_i$ depends on whether $\cO_i$ is relevant, marginal, or irrelevant, i.e.\ whether $d-\De_i$ is positive, zero, or negative. 
