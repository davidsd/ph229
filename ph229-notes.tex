...
To describe more complicated operators, instead of starting with the conformal Ward identities, let us try to guess what they lift to.
...
To translate from the embedding space back to $\R^d$, we have the following dictionary
\be
\label{eq:embeddingtoflatspace}
V_\mu(x) &= \pdr{X_A(x)}{x^\mu} \mathcal{V}^A(X)|_{X=(1,x^2,x)} \nn\\
...
Let us do the same exercise, this time for an operator $\cO^a$ in a nontrivial irreducible representation $\rho$ of $\SO(d)$. We normalize $\cO$ so that
\be
\<\cO_b|\cO^a\> &= \de^a_b.
\ee
...
\ee
where $(L^{\a\b})_{\mu\nu}$ is the generator of rotations in the vector representation $\myng{(1)}$.  Writing $A=\a\b$ for an adjoint index of $\SO(d)$, and thinking of $L^A,\cS_A$ as operators on $\myng{(1)}\otimes \rho$, this becomes
\be
L^A \cS_A &= \frac 1 2 \p{(L+\cS)^2-L^2 -\cS^2}\nn\\
&= \frac 1 2\p{-\mathrm{Cas}(\myng{(1)}\otimes \rho)+\mathrm{Cas}(\myng{(1)})+\mathrm{Cas}(\rho)},
\ee
...
Let's specialize to the case where $\rho= V_\ell$ is the spin-$\ell$ traceless symmetric tensor representation.  $V_\ell$ has Casimir $\ell(\ell+d-2)$.  To get the maximal eigenvalue of $L\.\cS$, we need the minimal Casimir of
\be
...
\be
\De &\geq \frac 1 2\p{-\mathrm{Cas}(V_{\ell-1})+\mathrm{Cas}(\myng{(1)})+\mathrm{Cas}(V_\ell)}\nn\\
&= \ell+d-2.
\ee
