...
If we allow ourselves to use the 2d $\e$-symbol, then there is actually an additional allowed tensor structure for $\<T_{\mu\nu}(x)T_{\rho\s}(0)\>$.\footnote{Exercise: find it!} Previously in section~\ref{sec:twoptsection} we proved that operators in an irreducible representation of $\SO(d)$ have a unique two-point structure. This is not a contradiction because the spin-$\ell$ representation in 2d is actually {\it reducible}. Specifically, $\SO(2)=U(1)$, and the spin-$\ell$ representation splits into two 1-dimensional irreps of $U(1)$ with charges $\pm \ell$. In the case of the stress tensor, these irreps are precisely described by $T(z)=T_{zz}(z)$ and $\bar T(\bar z) = T_{\bar z\bar z}(\bar z)$ --- they don't mix with each other under $\SO(2)$ rotations. Thus, it is possible to write two separate two-point structures 
\be
...
A 2d CFT has both a left-moving and right-moving Virasoro symmetry, so we can organize operators in the theory into primaries/descendants with respect to both Virasoro algebras.
...
\be
h_{r,s} &= \frac{c-1}{24} + \frac 1 4 (r \a_++s\a_-)^2 \nn\\
...
The three-point function $\<\cO_1(z_1)\cO_2(z_2)\cO(z)\>$ is equal to the first term in (\ref{eq:twodthreept}). It is not strictly-speaking a function of $z_1,z_2,z$ because it is not single-valued in general (since the $h_i$ can take non-integer values). Only after combining it with the antiholomorphic term in (\ref{eq:twodthreept}), do we get a single-valued function in two dimensions, due to the condition $h-\bar h\in \Z$. Similarly, the Virasoro block $\mathcal{F}^{34}_{12}(h|z_i)$ is not actually a single-valued function of the $z_i$. It has interesting monodromies that we will discuss later.

A single unitary Virasoro multiplet splits into a direct sum of global conformal multiplets. Consequently, the projector onto a Virasoro multiplet can be expressed as a sum of projectors onto global ($\SL(2,\R)$) multiplets
\be
\label{eq:virasoroprojectorglobal}
|\cO|_\mathrm{Vir} &= \sum_{\{k\},\{k'\}} |\cO,\{k\}\> \cM_{\{k\},\{k'\}}^{-1} \<\cO,\{k'\}| \nn\\
&= \sum_{\cO'} |\cO'|_\mathrm{global}.
\ee
Here, $\cO'$ runs over all Virasoro descendants of $\cO$ that are primaries with respect to $\SL(2,\R)$. In other words, $L_{1}|\cO'\>=0$, but the higher $L_{m}$'s do not necessarily kill $\cO'$. Besides the Virasoro primary itself, the first example of such a state occurs at level 2. We can find it by solving
\be
0 &= L_1(L_{-2}+ aL_{-1}^2)|\cO\> \nn\\
 &= (3+2 a (2h+1)) L_{-1} |\cO\> \quad \implies \quad a = - \frac{3}{4h+2}
\ee
Thus, the first $\SL(2,\R)$ primary state is
\be
\label{eq:exprforleveltwoprim}
|\cO'\> &= \p{L_{-2} - \frac{3}{4h+2} L_{-1}^2}|\cO\>
\ee
...
As a check on (\ref{eq:exprforleveltwoprim}), we can compute a three-point function
\be
\<\cO_1(z_1)\cO_2(z_2) \cO'(z_0)\> &= \p{\cL_{-2} - \frac{3}{4h+2} \cL_{-1}^2}\<\cO_1(z_1)\cO_2(z_2) \cO(z_0)\>,
\ee
For simplicity, consider the case $h_1=h_2=\de$. Plugging in the standard form for a three-point function of primaries, we find after some impressive cancellations
\be
...
\ee
The result indeed has the form of a three-point function of global primaries, times an interesting coefficient.

As a consequence of (\ref{eq:virasoroprojectorglobal}), Virasoro blocks are a sum of global conformal blocks. The relative coefficients of the different blocks are determined by three-point functions with different Virasoro descendants of $\cO$. Furthermore, when $h,c$ obey unitarity, the sum over global blocks has positive coefficients.
...
\ee
An operator insertion at infinity is defined by $\cO(\oo)=\lim_{L\to \oo} L^{2h} \cO(L)$. Note that $\cO_1(0)|0\>=|\cO_1\>$ and $\<0|\cO_4(\oo)=\<\cO_4|$.
...
\ee
and a similar expression for $\<\cO_3\cO_4\cO\>$. Let us compute them one by one. We have
\be
...
\ee
In the $z_4\to \oo$ limit, the term with $i=4$ will be subleading. Thus, we have
\be
...
Let us use this inside a matrix element 
\be
\<\cO|L_{\{k\}} \cO_2(z) |\cO_1\> &= z^{-h_2}\<\cO|L_{\{k\}} z^{L_0} \cO_2(1) z^{-L_0} |\cO_1\> \nn\\
&= z^{-h_2-h_1+h_\cO+N} \<\cO|L_{\{k\}} \cO_2(1)|\cO_1\>
\ee
where $N$ is the level of $L_{\{k\}}$. Now the remaining matrix element is just
\be
...
Let us make some observations about this result. The Virasoro block is equal to $z^{h-2\de}$ times a power series in $z$, where each integer power corresponds to the level of the descendent being ``exchanged" between $\cO_1\cO_2$ and $\cO_3 \cO_4$. The coefficients in this power series are rational functions of $h,\de$, and $c$. The Virasoro block should be expressible as a linear combination of $\SL(2,\R)$ blocks corresponding to the global primaries
\be
...
Subtracting off the first $\SL(2,\R)$ block, we can deduce the next term in the expansion in global blocks
\be
...
\ee
Indeed, the crazy factor out front is just the square of the coefficient~(\ref{eq:checkonleveltwoprim}) divided by the norm-squared of the level-2 global primary
\be
\label{eq:normglobalprim}
\left|\p{L_{-2} - \frac{3}{4h+2} L_{-1}^2}|\cO\>\right|^2 &= \frac{c(2h+1)+2h(8h-5)}{2(2h+1)}.
\ee
...
Because all of the global primaries besides $|\cO\>$ itself contain higher Virasoro generators $L_{-k}$ with $k>1$, their norms are polynomials in $c$ with nonzero degree. In the large-$c$ limit, the contribution of these states goes away, and we are left with only the leading global block $z^{-2\de} k_{2h}(z)$. This leading block can be computed using the Casimir equation for $\SL(2,\R)$ as we discussed before, or by explicitly summing descendants with respect to $L_{-1}$. Indeed, it's not too hard to improve our previous calculation to do this to all orders. We need
\be
...
\ee
where $(h)_n=h(h+1)\cdots(h+n-1)=\frac{\Gamma(h+n)}{\Gamma(h)}$ is the Pochhammer symbol.  We also need the result
\begin{exercise}
...
\ee
\end{exercise}
Thus, the $\SL(2,\R)$ block is
\be
\sum_{n=0}^\oo \frac{(h)_n^2}{n!(2n)_n} z^{h+n-2\de} = z^{-2\de} k_{2h}(z).
\ee
